...
# 红绿灯判定参数
GREEN_LIGHT_STABLE_FRAMES = 1  # 绿灯稳定帧数
# 关键语音保护窗：避免"绿灯稳定，开始通行。"被下一条方向语音立刻覆盖
CRITICAL_VOICE_PROTECT_SEC = float(os.getenv('CROSSWALK_CRITICAL_VOICE_PROTECT_SEC', '2.5'))
...
class CrossStreetNavigator:
    ...
    def __init__(self, seg_model=None, obs_model=None, device_id: str = "esp32"):
        ...
        self.critical_voice_until = 0.0      # 关键语音保护截止时间（防止被方向语音覆盖）
        ...
        try:
            ...
            if self.state == STATE_SEEKING:
                        ...
                        if result and 'stable_light' in result:
                            ...
                            if stable_light == 'go':
                                ...
                                if self.green_light_counter >= GREEN_LIGHT_STABLE_FRAMES:
                                    ...
                                    # 开启关键语音保护窗，避免下一帧方向语音覆盖
                                    self.last_guide_time = current_time
                                    self.critical_voice_until = current_time + CRITICAL_VOICE_PROTECT_SEC
                                    self.green_light_counter = 0
                ...
                if crosswalk_mask is not None:
                    ...
                    # 语音输出（节流）
                    if current_time - self.last_guide_time > self.guide_interval:
                        in_critical_voice_window = current_time < self.critical_voice_until
                        # 检查是否快走完斑马线
                        ...
                        # 优先级4：障碍物
                            ...
                            self.last_guide_time = current_time
                        # 优先级5：方向引导
                        elif not in_critical_voice_window:
                            guidance_text = direction
                            self.last_guide_time = current_time
                        else:
                            logger.info("[CROSS_STREET] 关键语音保护窗生效，跳过方向播报")
                else:
